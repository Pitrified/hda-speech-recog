...
% High level description of the processing flow:
...
% the spirit of the dataset as a tool to enable meaningful
...
\subsubsection{LibriTTS dataset}
...
The command recognition problem consists in identifying which word is spoken in
...
\tab{tab:task_word_composition} lists the composition of each task.
%
...
% MAYBE: Use Pool, MAYBE in conclusion/what I learned section.
...
\end{itemize}
% MAYBE write better info for exp_decay 
...
% MAYBE: a block model would be nice
...
(\fig{fig:attention_weights_area}), while VerticalAreaNet builds a $(1,
(h/16))$ grid to weigh the input data, (\fig{fig:attention_weights_vertical}).
