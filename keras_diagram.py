...
class Node:
    ...
    def _calculate_children(self):
        ...
        layers = list(
            _flatten([node.inbound_layers for node in self.layer._inbound_nodes])
        )
        # XXX: ends here
        layers = [ll.layers[-1] if issubclass(type(ll), Model) else ll for ll in layers]
        return [Node(ll) for ll in layers]
    ...
    def canvas(self):
        ...
        for child in self.children:
            ...
            arrows.append(
                (offset + _center_of(child.family_width), _center_of(self.family_width))
            )
            offset += c.width()
        if len(arrows) > 1:
...
class Canvas:
    ...
    def append_to_right(self, canvas):
        ...
        self.chars[
            (self.height() - canvas.height()) :, self.width() - canvas.width() :
        ] = canvas.chars
